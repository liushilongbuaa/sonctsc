...
do
    ...
    do
        ...
        if [[ "$bypass_year" == n ]] && [ -z "$(ls $year/$repo.*.$dump_type.json 2>/dev/null)" ];then
            ...
            rm -rf $file_by_year
            eval $debug gh pr list -R $org_repo -L 10000 -s merged --json $keys -S "merged:$year-01-01..$year-12-31" | jq --indent 4 ".[] += {repo: \"$repo\"} | sort_by(.number)"  > $file_by_year
            sleep $interval
            check_dump_file $file_by_year && continue
        fi
        ...
        do
            ...
            sleep $interval
            jq -s 'add' --indent 4 $year/$repo.$month.a.$dump_type.json $year/$repo.$month.b.$dump_type.json $year/$repo.$month.c.$dump_type.json > $year/$repo.$month.$dump_type.json
            rm $year/$repo.$month.a.$dump_type.json $year/$repo.$month.b.$dump_type.json $year/$repo.$month.c.$dump_type.json
        done    
